// Test Matrix & Sprint Tracker Data
// Source: FIZ Creative Playbook - Creative Testing Framework

// Test variables (levers)
export const testVariables = {
  hook: {
    id: 'hook',
    name: 'Hook',
    description: 'Opening message/angle',
    impact: 'CTR + CVR',
    priority: 1,
    options: [
      { id: 'calm', name: 'Calm', example: 'Mais calma. Menos burocracia.' },
      { id: 'control', name: 'Control', example: 'Controlo total, sem ser contabilista.' },
      { id: 'speed', name: 'Speed', example: 'Fatura feita em segundos.' },
      { id: 'protection', name: 'Protection', example: 'Um escudo contra coimas.' }
    ]
  },

  offer: {
    id: 'offer',
    name: 'Offer',
    description: 'Main value proposition',
    impact: 'CVR (strongest)',
    priority: 1,
    options: [
      { id: 'autopilot', name: 'Autopilot', example: 'IVA + SS no automático' },
      { id: 'escudo', name: 'Escudo Fiscal', example: 'Cobertura até 1.500€' },
      { id: 'free', name: '0€ Forever', example: 'Faturação grátis para sempre' },
      { id: 'whatsapp', name: 'WhatsApp-first', example: 'Cria faturas pelo WhatsApp' }
    ]
  },

  visual: {
    id: 'visual',
    name: 'Visual Style',
    description: 'Design direction',
    impact: 'CTR (click quality)',
    priority: 3,
    options: [
      { id: 'premium', name: 'Premium Protection', example: 'Dark + shield + metallic' },
      { id: 'minimal', name: 'Minimal Autopilot', example: 'Light + clean + Stripe-like' },
      { id: 'whatsapp', name: 'WhatsApp Friendly', example: 'Chat bubbles + warm' }
    ]
  },

  proof: {
    id: 'proof',
    name: 'Proof Layer',
    description: 'Trust element',
    impact: 'CVR',
    priority: 2,
    options: [
      { id: 'certified', name: 'Certificada', example: 'Certificada pela AT / SAF-T' },
      { id: 'terms', name: 'Escudo + Termos', example: 'até 1.500€ (ver termos)' },
      { id: 'whatsapp-demo', name: 'WhatsApp Demo', example: 'Chat screenshot' },
      { id: 'built-pt', name: 'Built for PT', example: 'Feito para Portugal' }
    ]
  },

  cta: {
    id: 'cta',
    name: 'CTA',
    description: 'Call to action text',
    impact: 'CVR (micro)',
    priority: 4,
    options: [
      { id: 'start-free', name: 'Start Free', example: 'Começar grátis' },
      { id: 'autopilot-on', name: 'Enable Autopilot', example: 'Ativar Autopiloto' },
      { id: 'use-whatsapp', name: 'Use WhatsApp', example: 'Usar no WhatsApp' },
      { id: 'see-how', name: 'See How', example: 'Ver como funciona' }
    ]
  }
};

// Recommended test order
export const testOrder = [
  { step: 1, variables: ['offer', 'hook'], reason: 'Biggest CPA impact' },
  { step: 2, variables: ['proof'], reason: 'Reduces doubts' },
  { step: 3, variables: ['visual'], reason: 'Balance trendy vs clear' },
  { step: 4, variables: ['cta'], reason: 'Micro optimization' }
];

// 7-day sprint template
export const sprintTemplate = {
  day1: {
    name: 'Launch',
    tasks: [
      'Launch 6-8 creatives (2 hooks × 2 offers × 2 styles)',
      'Set up tracking (UTMs, pixels)',
      'Document baseline metrics'
    ],
    metrics: ['Impressions', 'CTR', 'CPC']
  },
  day2: {
    name: 'First Cut',
    tasks: [
      'Review CTR and CPC data',
      'Kill obvious losers (low CTR + bad CPC)',
      'Note top 2 performers'
    ],
    metrics: ['CTR', 'CPC', 'CPM']
  },
  day3: {
    name: 'Iterate',
    tasks: [
      'Create 6 new variations around top-2 winners',
      'Change only 1 variable per variation',
      'Launch new batch'
    ],
    metrics: ['CTR', 'CPC', 'Early CVR signals']
  },
  day4: {
    name: 'Proof Test',
    tasks: [
      'Add proof variants (certificada vs escudo termos vs WhatsApp demo)',
      'Test against winning hooks/offers'
    ],
    metrics: ['CVR', 'CPA']
  },
  day5: {
    name: 'Consolidate',
    tasks: [
      'Identify winning combinations',
      'Scale production of winning templates',
      'Create mass variations'
    ],
    metrics: ['CVR', 'CPA', 'ROAS']
  },
  day6: {
    name: 'Scale',
    tasks: [
      'Increase budget on winners',
      'Pause underperformers',
      'Document learnings'
    ],
    metrics: ['CPA', 'ROAS', 'Volume']
  },
  day7: {
    name: 'Review & Plan',
    tasks: [
      'Fix "champion template set"',
      'Plan next sprint focus',
      'Archive test data'
    ],
    metrics: ['CPA trend', 'Best performers list']
  }
};

// KPI benchmarks (example, adjust to actual data)
export const kpiBenchmarks = {
  ctr: { good: 1.5, great: 2.5, unit: '%' },
  cpc: { good: 0.30, great: 0.15, unit: '€' },
  cpm: { good: 5.00, great: 3.00, unit: '€' },
  cvr: { good: 2.0, great: 4.0, unit: '%' },
  cpa: { good: 15.00, great: 8.00, unit: '€' }
};

// Pre-built test matrices
export const prebuiltMatrices = [
  {
    id: 'starter',
    name: 'Starter Pack',
    description: 'Best for new campaigns - test core offer angles',
    variables: ['hook', 'offer'],
    combinations: 8,
    estimated: '2-3 days to insights'
  },
  {
    id: 'full',
    name: 'Full Matrix',
    description: 'Comprehensive test across all variables',
    variables: ['hook', 'offer', 'visual', 'proof'],
    combinations: 48,
    estimated: '5-7 days to insights'
  },
  {
    id: 'visual-focus',
    name: 'Visual Focus',
    description: 'When you have a winning message, test visuals',
    variables: ['visual', 'proof'],
    combinations: 9,
    estimated: '2-3 days to insights'
  },
  {
    id: 'icp-split',
    name: 'ICP Split Test',
    description: 'Same creative, different audience messaging',
    variables: ['hook', 'offer'],
    icpSplit: true,
    combinations: 12,
    estimated: '3-4 days to insights'
  }
];

// Generate matrix combinations
export function generateMatrix(selectedVariables) {
  const vars = selectedVariables.map(v => testVariables[v]);

  function cartesian(arrays) {
    return arrays.reduce((acc, arr) =>
      acc.flatMap(x => arr.map(y => [...x, y])),
      [[]]
    );
  }

  const optionArrays = vars.map(v => v.options.map(o => ({ variable: v.id, ...o })));
  const combinations = cartesian(optionArrays);

  return combinations.map((combo, i) => ({
    id: `V${String(i + 1).padStart(2, '0')}`,
    elements: combo,
    status: 'pending' // pending, active, winner, loser
  }));
}
